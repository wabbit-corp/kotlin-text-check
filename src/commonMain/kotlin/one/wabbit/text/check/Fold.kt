...
interface Fold<I, O> {
    interface WithState<I, S, O> : Fold<I, O> {
        ...
        fun <I1> mapInput(f: (I1) -> I): Fold<I1, O> {
            ...
            return object : Fold.WithState<I1, S, O> {
                ...
                override fun append(state: S, input: Iterator<I1>): S =
                    self.append(state, input.map(f))
        ...
        fun <S1, O1> zip(
            other: Fold.WithState<I, S1, O1>
        ): Fold.WithState<I, PairVar<S, S1>, Pair<O, O1>> {
            val self = this
            return object : Fold.WithState<I, PairVar<S, S1>, Pair<O, O1>> {
                ...
                override fun finalize(state: PairVar<S, S1>): Pair<O, O1> =
                    Pair(self.finalize(state.first), other.finalize(state.second))
            }
        }
    }
    ...
    companion object {
        val intSum: Fold<Int, Int> =
            object : Fold.WithState<Int, IntVar, Int> {
